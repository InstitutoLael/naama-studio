...
const WhatsAppPopup = () => {
  ...
  const joinVIP = () => {
    window.open(
      'https://wa.me/56979520623?text=Hola! Quiero unirme a la lista VIP de Naamá Studio para recibir promociones y novedades.',
      '_blank'
    );
    dismiss();
  };
  ...
  return (
    <div className="wa_popup_backdrop" onClick={dismiss} aria-modal="true" role="dialog">
      <div
        className="wa_popup_card"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Cerrar */}
        <button className="wa_close" onClick={dismiss} aria-label="Cerrar">
          <X size={16} strokeWidth={1.5} />
        </button>

        {/* Header */}
        <div className="wa_header">
          <div className="wa_icon_wrap">
            <WhatsAppIcon />
          </div>
        ...
        {/* Acciones */}
        <div className="wa_actions">
          <button className="wa_btn_primary" onClick={joinVIP}>
            Quiero unirme
          </button>
          <button className="wa_btn_ghost" onClick={dismiss}>
            Quizás más tarde
          </button>
        </div>
      </div>
